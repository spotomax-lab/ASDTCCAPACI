...
const BookingScreen = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // Per prenotazioni open, il creatore è l'unico giocatore confermato inizialmente
      const players = [
        {
          userId: user.uid,
          userName: user.displayName || user.email,
      ...
      if (bookingMode === 'standard') {
        ...
        for (const playerId of selectedPlayers) {
          const player = availablePlayers.find(p => p.id === playerId);
          const message = `${user.displayName || user.email} ti ha invitato a una partita ${matchType === 'singles' ? 'singolare' : 'doppio'} open il ${formatDate(selectedDate)} dalle ${selectedSlot.start} alle ${selectedSlot.end}`;
