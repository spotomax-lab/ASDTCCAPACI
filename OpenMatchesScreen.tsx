...
const OpenMatchesScreen = () => {
  ...
  // Funzione per verificare se una partita è passata
  const isPartitaPassata = (partita) => {
    try {
      const now = new Date();
      const dataPartita = new Date(partita.date);
      const [ore, minuti] = partita.startTime.split(':').map(Number);
      dataPartita.setHours(ore, minuti, 0, 0);
      
      return dataPartita < now;
    } catch (error) {
      console.error('Errore nel controllo data partita:', error);
      return false;
    }
  };
    ...
    try {
      ...
      unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          // Filtra le partite passate
          const partiteFiltrate = partiteList.filter(partita => 
            !isPartitaPassata(partita)
          );
          
          setPartiteOpen(partiteFiltrate);
          setLoading(false);
          setRefreshing(false);
          setError(null);
        },
  ...
  const handleJoinMatch = async (partita) => {
    ...
    // Controlla se la partita è passata
    if (isPartitaPassata(partita)) {
  ...
  if (partiteOpen.length === 0) {
    return (
      <View style={styles.vuotoContainer}>
        ...
        <Text style={styles.vuotoSottoTesto}>Le partite open future appariranno qui</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.sottoTitolo}>
        ...
        {"\n"}
        Le partite passate non vengono visualizzate
      </Text>
      <FlatList
        data={partiteOpen}
