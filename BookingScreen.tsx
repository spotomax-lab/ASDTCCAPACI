...
const BookingScreen = () => {
  ...
  const [userBookingsThisWeek, setUserBookingsThisWeek] = useState(0);
  ...
  // Funzione per ottenere la data di inizio della settimana (lunedì)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      setUserBookingsThisWeek(querySnapshot.size);
    });
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where('start', '>=', new Date(`${dateString}T00:00:00`)),
      where('start', '<=', new Date(`${dateString}T23:59:59`))
    );
  ...
  // Check if a time slot is blocked and return the type
  const getSlotBlockType = (slot) => {
    ...
    const blockedSlot = blockedSlots.find(block => {
      // Estrai l'ID del campo dal nome (es. "Campo 1" -> "1")
      const courtId = selectedField.replace('Campo ', '');
      
      // Controlla se il blocco è per questo campo
      if (block.courtId !== courtId) return false;
      
      // Controlla se lo slot è dentro l'intervallo bloccato
  ...
  const handleSlotPress = (slot) => {
    ...
    if (blockType) {
      const blockedReason = getBlockedReason(slot);
      Alert.alert(
        'Slot bloccato',
        blockedReason,
        [{ text: 'OK' }]
  ...
  const handleBooking = async () => {
    ...
    if (!user) {
      Alert.alert('Errore', 'Devi essere loggato per effettuare una prenotazione.');
      return;
    }

    // Check booking limit for non-admin users
    if (!isAdmin && userBookingsThisWeek >= 3) {
      Alert.alert(
        'Limite prenotazioni raggiunto',
        'Hai già effettuato 3 prenotazioni questa settimana. Non puoi effettuarne altre.'
      );
      return;
    }
  ...
  // Funzione per ottenere il colore dello slot in base al tipo
  ...
  // Funzione per ottenere il colore del testo dello slot in base al tipo
  const getSlotTextStyle = (slot) => {
    const blockType = getSlotBlockType(slot);
    const isBooked = isSlotBooked(slot);
    const isSelected = selectedSlots.includes(slot.start);
    ...
    if (blockType) {
      return [styles.timeSlotText, styles.timeSlotTextBlocked];
    }
    
    return [styles.timeSlotText];
  };
      ...
      >
        ...
        {!isAdmin && (
          <View style={styles.bookingLimitContainer}>
            <Text style={styles.bookingLimitText}>
              Prenotazioni questa settimana: {userBookingsThisWeek}/3
            </Text>
          </View>
        )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.instructionsContainer}>
            ...
            <Text style={styles.instructionItem}>• <Text style={styles.blueText}>Blu</Text> = Scuola Tennis</Text>
            <Text style={styles.instructionItem}>• <Text style={styles.orangeText}>Arancione</Text> = Lezioni Individuali</Text>
            <Text style={styles.instructionItem}>• <Text style={styles.redText}>Rosso</Text> = Bloccato (manutenzione)</Text>
            <Text style={styles.instructionItem}>• <Text style={styles.violetText}>Viola</Text> = Selezionato</Text>
            <Text style={styles.instructionItem}>• <Text style={styles.blueText}>Tocca</Text> gli slot prenotati/bloccati per vedere i dettagli</Text>
          </View>
        </View>
            ...
            {timeSlots.map((slot, index) => {
              ...
              return (
                ...
                >
                  ...
                  {blockType === 'school' && (
                    <Text style={styles.schoolIndicator}>🎾</Text>
                  )}
                  {blockType === 'individual' && (
                    <Text style={styles.individualIndicator}>👤</Text>
                  )}
                  {blockType === 'blocked' && (
                    <Text style={styles.blockedIndicator}>🔧</Text>
                  )}
                </TouchableOpacity>
              );
            })}
        ...
        <TouchableOpacity 
          ...
          disabled={bookingLoading || (!isAdmin && userBookingsThisWeek >= 3)}
        >
          {bookingLoading ? (
