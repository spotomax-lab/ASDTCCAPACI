...
export default function SlotConfigurationScreen() {
  ...
  const openEditor = (index: number) => {
    ...
    const notes = typeof current.notes === 'string' ? current.notes : (tmpNotes || '');
    ...
    if (end) {
      // If an explicit end time exists, use it and derive duration
      endVal = end;
      durationVal = Math.max(15, toMin(endVal) - toMin(startVal)); // at least 15'
    } else if (durFromCfg) {
      durationVal = durFromCfg;
      endVal = toHHMM(toMin(startVal) + durationVal);
    } else {
      // Fallback to 60'
      durationVal = 60;
      endVal = toHHMM(toMin(startVal) + durationVal);
