...
const CalendarView = ({ 
  ...
  // Funzione per renderizzare le icone dei giocatori (copiata da BookingScreen)
  ...
  const getSlotStyle = (slotInfo, isSelected = false) => {
    if (isSelected) {
    ...
    // MODIFICA: Le prenotazioni open confermate diventano verdi
  ...
  const handleSlotPress = (slot, slotInfo) => {
    if (isEditMode) {
      ...
      if (slotInfo.type !== 'free' && slotInfo.bookingData) {
        ...
        if (slotInfo.title) {
          message += `Motivo: ${slotInfo.title}`;
        }
      } else {
        // Messaggio per slot liberi
        message = `Orario: ${slot.display}\nCampo: ${selectedField}\nStato: Libero`;
      }
    ...
    <View>
      ...
      <View style={calendarStyles.timeGrid}>
        {timeSlots.map((slot, index) => {
          ...
          const slotStyle = getSlotStyle(slotInfo, isSelected);
          const booking = slotInfo.bookingData;
